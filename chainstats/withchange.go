...
// Updated to account for the fact that change is not related to any currency

func (bp *BehaviourPrice) AnalyzeDataWithChange(chain chainreadinterface.IBlockChain,
	...
	resultsChan := make(chan struct{}, numWorkers) // (Empty) work comes out here
	...
	for w := 0; w < numWorkers; w++ {
		g.Go(func() error { // Use the errgroup instead of "go func() {"
			...
			for blockBatch := range blockBatchChan {
				...
				satsArrayTransIndices := make([]uint64, 0, 1000) // Indices into satsArray for each transaction
					...
					for peel := 0; peel < numPeels; peel++ {
						...
						for log10RateBinNumber, _ := range behaviourModel.Bins {
							...
							for trans, satsIndex := range satsArrayTransIndices {
								...
								// Second loop... the actual probability we're interested in, P(transaction | rate)
								...
								for _, sats := range amounts {
									...
									if celebrity {
										...
										// The first term represents the possibility that the amount is the change
										firstTermLog := probProductLog - probLog // Divide is subtract for logs
										firstTermLog += logOneOverN              // Divide by N

										firstTerm := math.Exp(firstTermLog)
										secondTerm := math.Exp(probLog)
										thirdTerm := math.Exp(probProductLog)

										termInsideProduct := firstTerm + secondTerm - thirdTerm
										termInsideProductLog := math.Log(termInsideProduct)

										probTransLog += termInsideProductLog
									}
								} // for sats in amounts
								probRateScoreLog += probTransLog
